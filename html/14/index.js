...
function Scroll(pobj,obj,speed,interval){ //父级容器，轮播速度，切换间隔
    var $pobj=$('.'+pobj);
    $("."+obj).each(function(){
        var $box = $(this),
        ...
        function scroll(){               //轮播事件
            $imgUl.stop().animate({left:-width},speed,function(){
                ...
                $pobj.removeClass().addClass('banner').addClass("bg"+k);
            });
        }
        $btnLi.click(function(){            //小圆点点击事件
            var index = $btnLi.index(this);
            $(this).addClass('cur').siblings("li").removeClass('cur');
            $pobj.removeClass().addClass('banner').addClass('bg'+index);
            if(index >= k){
                var dif = index-k;
                left = -dif*width;
...
$(function () {                  //读取轮播事件
    Scroll("banner","bannerCon",600,2000);
